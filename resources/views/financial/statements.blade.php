<x-app-layout>
    <div class="container">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                ...
                <tbody>
                    @foreach ($financial->statements as $statement)
                        <tr class="bg-white">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $statement->information }}</th>
        ...
        <div id="modal" class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 flex hidden">
            <div class="bg-white rounded-lg shadow-lg p-6 w-96">
                ...
                <form action="{{ route('financial.statement.store', $financial->id) }}" method="POST">
                    @csrf
                    <input type="text" name="information" placeholder="Financial Information" required class="border border-gray-300 p-2 mb-2 w-full">
                    <input type="text" name="debit" placeholder="Debit" class="border border-gray-300 p-2 mb-2 w-full">
                    <input type="text" name="credit" placeholder="Credit"  class="border border-gray-300 p-2 mb-2 w-full">
